...
export default function AuthAdminPage() {
    ...
    const getRoleColor = (role: string) => {
        switch (role) {
            case 'MASTER': return 'text-sub-seoultech-red'
            case 'ADMIN': return 'text-sub-seoultech-blue'
            case 'MEMBER':
            case 'GUEST': return 'text-pri-gray-disabled'
...
function AuthItem({ id, name, role, part, onRoleChange, isUpdating }: AuthItemProps) {
    const getRoleDisplayName = (role: string) => {
        switch (role) {
            case 'MASTER': return '마스터'
            case 'ADMIN': return '관리자'
        ...
        <div className="bg-pri-gray-1 rounded-15 flex w-full flex-row items-center justify-between gap-36 px-36 py-36">
            ...
            <div className="flex items-center gap-16">
                <select
                    ...
                    disabled={isUpdating}
                    className="text-20 font-bold border-none bg-transparent cursor-pointer"
                >
...
function AuthList({
    ...
    return (
        <div className={cn('flex h-full w-full flex-col gap-16', className)}>
            ...
            {items.length === 0 && (
                <div className="text-center text-pri-gray-5">멤버가 없습니다.</div>
            )}
        </div>
    )
}
